...
void GraphMaker()
{
    ...
    double CrosstalkParametersX[2]={};
    double CrosstalkParametersY[2]={};
    
    PercentageGraphX->Fit("pol0", "Q");
    PercentageGraphY->Fit("pol0","Q");

    TF1 *funkcja1 = (TF1*)PercentageGraphX->GetListOfFunctions()->FindObject("pol0");
    TF1 *funkcja2 = (TF1*)PercentageGraphY->GetListOfFunctions()->FindObject("pol0");
    
    CrosstalkParametersX[0]=funkcja1->GetParameter(0);
    CrosstalkParametersX[1]=funkcja1->GetParError(0);
    
    CrosstalkParametersY[0]=funkcja2->GetParameter(0);
    CrosstalkParametersY[1]=funkcja2->GetParError(0);
    
    fstream plik;
    plik.open("/Users/kolos/Desktop/Studia/CIS/Crosstalk/StoppingProton/OutputGraph/Counter.txt", ios::out);
    plik<<"X percetage one cube  "<<CrosstalkParametersX[0]<<"X percetage error  "<<CrosstalkParametersX[1]<<endl;
    plik<<"Y percetage one cube  "<<CrosstalkParametersY[0]<<"Y percetage error  "<<CrosstalkParametersY[1]<<endl;
    ...
    TF1 *funkcja3 = PercentageOneGraphY->GetListOfFunctions()->FindObject("pol0");
    funkcja3->SetLineColor(4);
    ...
    TF1 *funkcja6 = PercentageConvertedTwoGraphY->GetListOfFunctions()->FindObject("pol0");
    funkcja6->SetLineColor(4);
     
    TCanvas *Canvas9 = new TCanvas("Canvas9","Canvas9", 1400, 1000);
    
    funkcja4->Draw("");
    funkcja6->Draw("SAME");
    funkcja4->GetXaxis()->SetTitle("Position from stopping point [cm]");
    funkcja4->GetYaxis()->SetTitle("Percentage %");
    funkcja4->GetYaxis()->SetRangeUser(0,4);
    gPad->Modified();

    auto legend10 = new TLegend(0.65,0.8,0.9,0.9);
    legend10->AddEntry(funkcja4,"X Axis","l");
    legend10->AddEntry(funkcja6,"Y Axis","l");
    legend10->Draw();
