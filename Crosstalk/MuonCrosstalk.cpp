  ...
  for (Int_t ih=0; ih < NumberEvDis;ih++){
    ...
    for (Int_t subSpill = 0; subSpill<minEn; subSpill++)
    {
        ...
        if (!SpillMised)
        {
            ...
            double CrosstalkDepositThreshold=5; //!!! wartosc testowa pewnie trzeba bedzie zwiekszyc
        ...
        if(LargehitTimeDif == 0 && event_XZ[eventNum]->GetEntries()>10)
        {
