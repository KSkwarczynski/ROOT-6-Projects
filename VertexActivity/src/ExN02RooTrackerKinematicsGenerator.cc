...
{  
  ...
  */
    ...
    if (inxml->GetXMLGenerTypeName()=="NEUT"){
    float targetWidth   = inxml->GetXMLSuperFGDCubeNum1_X(); // width along X
    float targetLength  = inxml->GetXMLSuperFGDCubeNum1_Z(); // length along Z
    float targetHeight  = inxml->GetXMLSuperFGDCubeNum1_Y(); // height along Y

    x = G4UniformRand() * targetWidth  - targetWidth*0.5  + inxml->GetXMLSuperFGDPos1_X();
    y = G4UniformRand() * targetHeight - targetHeight*0.5 + inxml->GetXMLSuperFGDPos1_Y();
    z = G4UniformRand() * targetLength - targetLength*0.5 + inxml->GetXMLSuperFGDPos1_Z();
