...
int SFGD_Reconstruction(int argc,char** argv) {
    ...
    for(int ievt=evtfirst;ievt<=EntryLast;ievt++){
        ...
        if(Nentries<100) WriteText = true;
        int PE_expect = 0;
        int PE_found = 0;
        ...
        for(int ihit=0;ihit<NHits;ihit++){ // get last entry
            ...
            int trkID = - 999;
            for (int m=0; m<7;m++) /////DLACZEGO TA PETLA?????
            {                
                int numPE_fiber = 0;
                    ...
                    if(VertexAnalysis>0 && ParticleCounter>0) //VertexActivityStudy
                    {
                        for(int ik=0; ik<ParticleCounter; ik++)
                        {
                            if(nd280UpHit->GetPrimaryId()==listOfParticles.at(ik))
                            {
                                ...
                                if(view == 2) hit->SetPE(0);
                                pex=0;
                                pey=0;
                                pez=0;
                    
                                if(WriteText)cout<<" UDALO SIE WYZEROWAC HIT"<<endl;
                            }
                        }
                    }
                    listOfHits.push_back(hit);
        ...
        for(uint vxl=0; vxl<listOfVoxels.size(); ++vxl){
            ...
            if(VertexAnalysis>0) //KAMIL
            {
                if(listOfVoxels[vxl]->GetTruePDGs()[0] == 13) //USUWANIE VOXLI
                {
                    ...
                    if(WriteText)cout<<" UDALO SIE WYZEROWAC"<<endl;
    ...
    h_shift->Fit("gaus");
    c_shift->Update();
    c_shift->SaveAs("/mnt/home/kskwarczynski/t2k-nd280-upgrade/Output/shift.pdf");
