...
void parseArguments()
{
    for (int iarg=0; iarg<gApplication->Argc(); iarg++)
    {
        if (string( gApplication->Argv(iarg))=="-h" || string( gApplication->Argv(iarg))=="--help" )
        {
            ...
            cout << "   -l || --ShortTrack  lenght of short track." << endl; //TODO
        ...
        else if (string( gApplication->Argv(iarg))=="-l") //TODO
        {
            iarg++;
            ShortTrack = atoi(gApplication->Argv(iarg));
        }
...
void linkFilesToTTrees(){
    ...
    data = (TTree*) FileInput->Get("AllEvents");
...
std::vector <ND280SFGDHit*> getEventMPPChits(int iev, int mode=0){
    ...
    if(IsMC) {
        ...
        for(Int_t ihit=0; ihit<unpackEvent->GetNHits(); ihit++){
            ...
            if(hit->GetDt() < dTimeIni || hit->GetDt() > dTimeFin) continue;
            if(mode!=1) if(hit->GetPE() <= 0 ) {jumpNextEvent = true; if(mode!=2) cout << "FIFO problem, jumping to next event." << endl; break;}
